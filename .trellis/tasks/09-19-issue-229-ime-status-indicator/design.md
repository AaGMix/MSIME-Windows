...
Caret lookup uses the focused context's current selection on the TSF owner thread but never mutates the real `TF_SELECTION.range`. The switch-anchor helper clones and collapses the selection to its active end, then probes cloned one-character ranges on both sides: the previous character contributes its right edge and the next character contributes its left edge. Compatible same-line LTR geometry resolves to the caret's physical bottom edge; newline boundaries may leave one unambiguous side. Cross-line, bidi/overlapping, clipped, degenerate, or failed geometry is rejected. An empty document may fall back to a usable collapsed extent. This avoids hosts such as Telegram returning a stale/high rectangle for a zero-length selection while leaving the correct non-empty composition-range candidate path unchanged. The lookup does not require an active composition and does not reuse COM objects across threads. If no usable anchor is available, the switch still updates normal state synchronization but no badge is shown.
