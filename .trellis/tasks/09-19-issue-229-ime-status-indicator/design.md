...
Placement reuses the candidate-window monitor, DPI, and work-area helpers. The badge is positioned above the text extent with a vertical clearance that keeps the caret and following text unobscured; it flips below only when the work area leaves no room above, then clamps inside the anchor monitor's work area. It recomputes size from the anchor monitor DPI on every show. Single-state badges remain icon-sized; punctuation badges use one fixed character-slot width for both Chinese and ASCII punctuation so their outer dimensions do not change when toggled. It is themed for light/dark mode and uses text rather than color as the sole state distinction.
