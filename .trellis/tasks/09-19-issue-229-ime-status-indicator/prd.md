...
- Keep the caret-indicator settings on the existing floating-toolbar page, but move its enable switch and position selector out of the floating-toolbar card into a separate caret-indicator card with its own preview; do not add another sidebar page. Lay out the four preview samples in one row when space permits and in two rows of two when constrained, never as four single-item rows; show a simulated text caret with every sample.
...
- [ ] The floating-toolbar settings page presents the caret indicator in a separate card containing its enable switch, position selector, and a skin-aware preview of `中`, `，。  中`, `全`, and `简`; each sample includes a simulated text caret, and the samples use one row when space permits or two rows of two when constrained, never four rows; the floating-toolbar card and preview contain only floating-toolbar controls/content.
